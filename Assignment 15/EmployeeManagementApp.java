...
class EmployeeID {
    public static int createEID() {
        while (true) {
            ...
            try {
		JdbcRowSet rs = DBConnection.getConnection();
                int employeeId = new Scanner(System.in).nextInt();
                rs.setCommand( "SELECT COUNT(*) FROM employees_assignment WHERE eid = ?");
		
		rs.setInt(1, employeeId);
		rs.execute();
		//ResultSet rst = rs.getResultSet();
...
class EmployeeFactory{
	...
	static{
		...
		try{
			JdbcRowSet rs = DBConnection.getConnection();
			rs.setCommand("select * from employees_assignment where designation='Ceo'");
			rs.execute();
...
class DBConnection{
	public static RowSetFactory rsf = null;
...
class EmpDAOImp implements EmpDAO{
	public void save(Employee emp){
		try{
			...
			rs.setInt(1,emp.getEID());
			rs.setString(2,emp.getName());
			rs.setInt(3,emp.getAge());
			rs.setFloat(4,emp.getSalary());
			rs.setString(5,emp.getDesignation());
			rs.setString(6,emp.getDepartment());
			rs.execute();
		}catch(Exception e){
			System.out.println(e);
		}
	...
	public void display(int choice){
		try{
			...
			switch(choice){
				case(1):{
					rs.setCommand("select * from employees_assignment order by eid");
					break;
				}
				case(2):{
					rs.setCommand("select * from employees_assignment order by NAME");
					break;
				}
				case(3):{
					rs.setCommand("select * from employees_assignment order by AGE");
					break;
				}
				case(4):{
					rs.setCommand("select * from employees_assignment order by SALARY");
					break;
				}
				case(5):{
					rs.setCommand("select * from employees_assignment order by DESIGNATION");
					break;
				}
			}
        		rs.execute();
	...
	public void find(int choice){
		try{
			...
			switch(choice){
				case(1):{
					rs.setCommand("select * from employees_assignment where Eid =?");
					System.out.print("Enter employee ID to search: ");
					int searchId = scanner.nextInt();
					rs.setInt(1,searchId);
					scanner.nextLine();
					break;
				}
				case(2):{
					rs.setCommand("select * from employees_assignment where NAME =?");
					System.out.print("Enter employee Name to search: ");
        	        	        String searchName = scanner.nextLine();	
					rs.setString(1,searchName);
					break;
				}
				case(3):{
					rs.setCommand("select * from employees_assignment where DESIGNATION =?");
					System.out.print("Enter employee Designation to search: ");
	                        	String searchDesignation = scanner.nextLine();
					rs.setString(1,searchDesignation);
					break;
				}
			}
        		rs.execute();
		...
		try{
			...
			while(delId==-1){
				...
				System.out.print("Are you sure you want to delete this record? (yes/no): ");
					...
					rs.setCommand("DELETE FROM employees_assignment WHERE eid = ?");
                    			rs.setInt(1, delId);
                    			rs.execute();
					Employee.inCount--;
		...
		try{
			...
			while(raiseId==-1){
				...
				if (confirm.equalsIgnoreCase("yes")) {
					...
					rs.setCommand("UPDATE employees_assignment SET salary = ? WHERE eid = ?");
